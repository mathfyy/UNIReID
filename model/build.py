...
class CLIP2ReID(nn.Module):
    def __init__(self, args, num_classes=11003):
        ...
        self.base_model, base_cfg = build_CLIP_from_openai_pretrained(args.pretrain_choice, args.img_size,
                                                                      args.stride_size, False, None, self.args.advance,
                                                                      self.args.add_token_num)
        ...
        if args.fusion_way == 'global concat':
            self.dim_conv = nn.Linear(512 * 2, 512)
        ...
        if 'mcm' in args.loss_names or 'mcq' in args.loss_names or 'mlm' in args.loss_names or 'msm' in args.loss_names:
            self.cross_attn = nn.MultiheadAttention(self.embed_dim,
                                                    self.embed_dim // 64,
                                                    batch_first=True)
            self.cross_modal_transformer = Transformer(width=self.embed_dim,
            ...
            # mcm
            if 'mcm' in args.loss_names:
                self.mcm_head = nn.Sequential(
                    OrderedDict([('dense', nn.Linear(self.embed_dim, self.embed_dim)),
                                 ('gelu', QuickGELU()),
                                 ('ln', LayerNorm(self.embed_dim)),
                                 ('fc', nn.Linear(self.embed_dim, args.num_colors))]))
                # self.mcm_head = nn.Linear(self.embed_dim, args.num_colors)
                # init mcm head
    ...
    def encode_image_text(self, image, text):
        x = self.base_model.encode_image_text(image, text)
        return x

    def encode_image_test(self, image):
        x = self.base_model.encode_image_test(image)
        return x

    def fusion_layer(self, text, sketch, caption_ids, pa=0.1, way='add'):

        if way == 'weight add':
            f_feats = self.gate[0] * text[torch.arange(text.shape[0]), caption_ids.argmax(dim=-1)] + self.gate[
                1] * sketch[:, 0, :]
        elif way == 'cross attention':
            f_feats = (self.cross_former(text[torch.arange(text.shape[0]), caption_ids.argmax(dim=-1)].unsqueeze(1),
                                         sketch, sketch) + self.cross_former(sketch[:, 0, :].unsqueeze(1), text, text))
            ...
            # f_feats = (self.cross_former(text,sketch,sketch)[:, 0, :] + self.cross_former(sketch,text,text)[torch.arange(text.shape[0]), caption_ids.argmax(dim=-1)])
            f_feats = self.cross_former(text[torch.arange(text.shape[0]), caption_ids.argmax(dim=-1)].unsqueeze(1),
                                        sketch, sketch).squeeze(1).contiguous()
        elif way == 'parameter add':
            f_feats = (1 - pa) * text[torch.arange(text.shape[0]), caption_ids.argmax(dim=-1)] + pa * sketch[:, 0, :]
        elif way == 'concat':
            f_feats = self.dim_conv(
                torch.cat((text[torch.arange(text.shape[0]), caption_ids.argmax(dim=-1)], sketch[:, 0, :]), dim=1))
            ...
            f_feats = f_feats[:, l_t:, :][:, 0, :] + f_feats[:, :l_t, :][
                torch.arange(text.shape[0]), caption_ids.argmax(dim=-1)]
    ...
    def forward(self, batch):
        ...
        if self.args.only_sketch:
            i_feats = image_feats[:, 0, :].float()
            si_feats = simage_feats[:, 0, :].float()
            ret.update(
                {'itc_loss': (objectives.compute_itc(i_feats, si_feats, logit_scale)) * self.args.cmm_loss_weight})
        elif self.args.only_text:
            i_feats = image_feats[:, 0, :].float()
            t_feats = text_feats[torch.arange(text_feats.shape[0]), caption_ids.argmax(dim=-1)].float()  # [64, 512]
            ret.update(
                {'itc_loss': (objectives.compute_itc(i_feats, t_feats, logit_scale)) * self.args.cmm_loss_weight})
        # elif self.args.only_fusion:
        #     i_feats = image_feats[:, 0, :].float()
        #     si_feats = simage_feats[:, 0, :].float()
        #     t_feats = text_feats[torch.arange(text_feats.shape[0]), caption_ids.argmax(dim=-1)].float() #[64, 512]
        ...
        #     ret.update({'itc_loss':(objectives.compute_itc(i_feats, f_feats, logit_scale))*self.args.cmm_loss_weight})
            ...
            if self.args.fusion_way in ['add', 'weight add', 'cross attention', 'parameter add', 'concat',
                                        'global concat', 'cross attention text', 'cross attention sketch',
                                        'concat transformer']:
                if self.args.advance is False or self.args.add_token_num == 0:
                    f_feats = self.fusion_layer(text_feats, simage_feats, caption_ids, pa=self.args.pa,
                                                way=self.args.fusion_way)
                else:
                    f_feats = torch.cat([text_feats[:, 0:self.args.add_token_num, :],
                                         simage_feats[:, 1:self.args.add_token_num + 1, :]], dim=1).float()
                    f_feats = torch.mean(f_feats, dim=1)
                    text_feats = text_feats[:, self.args.add_token_num:, :]
                ...
                if self.args.only_fusion_loss:
                    ret.update({'itc_loss': (objectives.compute_itc(i_feats, f_feats,
                                                                    logit_scale)) * self.args.cmm_loss_weight})
                elif self.args.four_fusion_loss:
                    ret.update({'itc_loss': (objectives.compute_itc(i_feats, t_feats,
                                                                    logit_scale) + objectives.compute_itc(i_feats,
                                                                                                          si_feats,
                                                                                                          logit_scale) + objectives.compute_itc(
                        i_feats, f_feats, logit_scale) + objectives.compute_itc(si_feats, t_feats,
                                                                                logit_scale)) * self.args.cmm_loss_weight})
                elif self.args.focal_three_fusion_loss3:
                    ret.update({'itc_loss': (objectives.compute_itc_focal3(i_feats, t_feats, si_feats, f_feats,
                                                                           logit_scale, self.args.al, self.args.ga,
                                                                           self.args.klp)) * self.args.cmm_loss_weight})
                else:
                    ret.update({'itc_loss': (objectives.compute_itc(i_feats, t_feats,
                                                                    logit_scale) + objectives.compute_itc(i_feats,
                                                                                                          si_feats,
                                                                                                          logit_scale) + objectives.compute_itc(
                        i_feats, f_feats, logit_scale)) * self.args.cmm_loss_weight})
            else:
                i_feats = image_feats[:, 0, :].float()
                si_feats = simage_feats[:, 0, :].float()
                t_feats = text_feats[torch.arange(text_feats.shape[0]), caption_ids.argmax(dim=-1)].float()  # [64, 512]
                ret.update({'itc_loss': (objectives.compute_itc(i_feats, t_feats, logit_scale) + objectives.compute_itc(
                    i_feats, si_feats, logit_scale)) * self.args.cmm_loss_weight})

        return ret

        # if 'tcmpm' in self.current_task:
        #     if self.args.use_imageid:
        #         ret.update({'tcmpm_loss':objectives.compute_tcmpm(i_feats, t_feats, batch['pids'], logit_scale, image_id=batch['image_ids'])*self.args.cmm_loss_weight})
        #     else:
        #         ret.update({'tcmpm_loss':objectives.compute_tcmpm(i_feats, t_feats, batch['pids'], logit_scale)*self.args.cmm_loss_weight})
        ...
        #     ret.update({'sdm_loss':objectives.compute_sdm(i_feats, t_feats, batch['pids'], logit_scale)*self.args.cmm_loss_weight})

        # if 'cmpm' in self.current_task:
        #     ret.update({'cmpm_loss':objectives.compute_cmpm(i_feats, t_feats, batch['pids'])*self.args.cmm_loss_weight})

        # if 'supcon' in self.current_task:
        #     bs, d = i_feats.size()
        #     i_feats = i_feats.view(-1, self.args.num_instance, d)
        #     si_feats = si_feats.view(-1, self.args.num_instance, d)
        #     t_feats = t_feats.view(-1, self.args.num_instance, d)
        #     f_feats = f_feats.view(-1, self.args.num_instance, d)
        #     label = label.view(-1, self.args.num_instance)[:,0]

        #     ret.update({'supcon_loss':(objectives.SupConLoss(torch.cat((i_feats, t_feats),dim=1), label)+objectives.SupConLoss(torch.cat((i_feats, si_feats),dim=1), label)+objectives.SupConLoss(torch.cat((i_feats, f_feats),dim=1), label))*self.args.cmm_loss_weight})
        ...
        #     scores = x.float().reshape(-1, self.args.num_colors)
        #     mcm_labels = batch['mcm_labels'].reshape(-1)
        #     ret.update({'mcm_loss': objectives.compute_mcm_or_mlm(scores, mcm_labels)*self.args.mcm_loss_weight})
        ...
        #     ret.update({'mcq_loss': objectives.compute_mcq(pred_answer_feats, answer_feats)*self.args.mcq_loss_weight})
