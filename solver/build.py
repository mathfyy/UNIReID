...
def build_optimizer(args, model):
    ...
    if args.advance is True:
        for i in range(model.base_model.resblocks_i.__len__()):
            for key, value in model.base_model.resblocks_i[i].named_parameters():
                key_all.append(key)
                if not value.requires_grad:
                    continue
                lr = args.lr
                ...
                params += [{"params": [value], "lr": lr, "weight_decay": weight_decay}]
            for key, value in model.base_model.resblocks_t[i].named_parameters():
                key_all.append(key)
                if not value.requires_grad:
                    continue
                lr = args.lr
