...
class Evaluator():
    ...
    def _compute_embedding(self, model):
        ...
        for simg, simage_id, pid, caption in self.txt_loader:
            ...
            simg = simg.to(device)
            with torch.no_grad():
                if self.args.advance is False:
                    text_feat = model.encode_text(caption)
                    sketch_feat = model.encode_image(simg)
                else:
                    sketch_feat, text_feat = model.encode_image_text(torch.cat([simg, simg], dim=0), caption)
                    sketch_feat = sketch_feat[sketch_feat.shape[0] // 2:, :, :]
                ...
                if self.args.fusion_way in ['add', 'weight add', 'cross attention', 'parameter add', 'concat',
                    ...
                    if self.args.advance is False or self.args.add_token_num == 0:
                        ...
                        text_sketch_fu = torch.cat([text_feat[:, 0:self.args.add_token_num, :],
                                                    sketch_feat[:, 1:self.args.add_token_num + 1, :]], dim=1).float()
                        text_sketch_fu = torch.mean(text_sketch_fu, dim=1)
                        text_feat = text_feat[:, self.args.add_token_num:, :]
        ...
        for pid, img, image_id in self.img_loader:
            ...
            with torch.no_grad():
                if self.args.advance is False:
                    ...
                    img_feat = model.encode_image_test(img)[:, 0, :].float()
                    # img_feat = model.encode_image_test(torch.cat([img, img], dim=0))
                    # img_feat = img_feat[:img_feat.shape[0] // 2, 0, :].float()
        ...
        for pid, simg, simage_id in self.sketch_loader:
            ...
            with torch.no_grad():
                if self.args.advance is False:
                    ...
                    simg_feat = model.encode_image_test(simg)[:, 0, :].float()
                    # simg_feat = model.encode_image_test(torch.cat([simg, simg], dim=0))
                    # simg_feat = simg_feat[simg_feat.shape[0] // 2:, 0, :].float()
    ...
    def eval(self, model, i2t_metric=False):
        ...
        qfeats_text, qfeats_sketch, qfeats_text_sketch, gfeats, qids, qids_sketch, gids, qimage_ids, qimage_ids_sketch, gimage_ids = self._compute_embedding(
            model)
        ...
        t2i_cmc, t2i_mAP, t2i_mINP = eval_func(-similarity_text_rgb.detach().cpu().numpy(), qids.numpy(), gids.numpy(),
                                               qimage_ids.numpy(), gimage_ids.numpy(), set=0, max_rank=10)

        # remove the rgb images that used for generated sketches from gallery set
        t2i_cmc0, t2i_mAP0, t2i_mINP0 = eval_func(-similarity_text_rgb.detach().cpu().numpy(), qids.numpy(),
                                                  gids.numpy(), qimage_ids.numpy(), gimage_ids.numpy(), set=2,
                                                  max_rank=10)

        t2i_cmc1, t2i_mAP1, t2i_mINP1 = eval_func(-similarity_sketch_rgb.detach().cpu().numpy(), qids_sketch.numpy(),
                                                  gids.numpy(), qimage_ids_sketch.numpy(), gimage_ids.numpy(), set=2,
                                                  max_rank=10)
        t2i_cmc2, t2i_mAP2, t2i_mINP2 = eval_func(-similarity_textsketch_rgb.detach().cpu().numpy(), qids.numpy(),
                                                  gids.numpy(), qimage_ids.numpy(), gimage_ids.numpy(), set=2,
                                                  max_rank=10)
        ...
        if i2t_metric:
            i2t_cmc, i2t_mAP, _ = rank(similarity=similarity_text_rgb.t(), q_pids=gids, g_pids=qids, max_rank=10,
                                       get_mAP=True)
